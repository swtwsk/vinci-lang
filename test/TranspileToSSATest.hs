...
cpsToSSATest = testGroup "CPS to SSA"
    ...
    where
        ...
        gSsa = SFnDef "f" [SArg "x"] (SBlock [SAssign "x0" $ SLitFloat 0.0, SGoto $ SLabel "g"]) 
            [ SLabelled (SLabel "g") [SPhiNode "i" [(SLabel "f_init", SArg "x0"), (SLabel "k1", SArg "res2")]] $ SBlock [SAssign "x10" $ SLitFloat 10.0, SAssign "res" $ SBinOp Ops.OpLT (SVar "i") (SVar "x10"), SIf (SVar "res") (SBlock [SGoto $ SLabel "k1"]) (SBlock [SGoto $ SLabel "k2"])]
