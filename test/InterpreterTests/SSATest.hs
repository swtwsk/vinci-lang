...
tests = testGroup "SSA tests"
    ...
    where
        gSsa = SFnDef "f" [SArg "x"] (SBlock [SAssign "x0" $ SLitFloat 0.0, SGoto $ SLabel "g"]) 
            [ SLabelled (SLabel "g") [SPhiNode "i" [(SLabel "f_init", SArg "x0"), (SLabel "k1", SArg "res2")]] $ SBlock [SAssign "x10" $ SLitFloat 10.0, SAssign "res" $ SBinOp OpLT (SVar "i") (SVar "x10"), SIf (SVar "res") (SLabel "k1") (SLabel "k2")]
