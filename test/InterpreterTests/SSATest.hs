...
tests = testGroup "SSA tests"
    ...
    where
        gSsa = SFnDef "f" TFloat [SArg $ Var "x" TFloat] (SBlock [SAssign (Var "x0" TFloat) $ SLitFloat 0.0, SGoto $ SLabel "g"]) 
            [ SLabelled (SLabel "g") [SPhiNode (Var "i" TFloat) [(SLabel "f_init", "x0"), (SLabel "k1", "res2")]] $ SBlock [SAssign (Var "x10" TFloat) $ SLitFloat 10.0, SAssign (Var "res" TFloat) $ SBinOp OpLT (SVar (Var "i" TFloat)) (SVar (Var "x10" TFloat)), SIf Nothing (SVar (Var "res" TFloat)) (SLabel "k1") (SLabel "k2")]
