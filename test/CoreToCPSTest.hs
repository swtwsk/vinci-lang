...
tests = testGroup "CoreToCPS tests" 
    ...
    where
        coreLambda = Core.Prog Core.NonRec "test" [] $ Core.LetFun (Core.Prog Core.NonRec "f" ["x"] (Core.Lit $ Core.LFloat 1.0)) (Core.App (Core.Var "f") (Core.Lit $ Core.LFloat 2.0))
        ...
        coreTuple = Core.Prog Core.NonRec "test" [] $ Core.Let "x" (Core.TupleCons [Core.Lit $ Core.LFloat 1.0, Core.Lit $ Core.LFloat 2.0]) (Core.TupleProj 0 (Core.Var "x"))
